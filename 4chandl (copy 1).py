...
def find_existing_thread(board_name, thread_id):
    ...
    board_dir = os.path.join(BASE_DOWNLOAD_DIR, board_name)
    if not os.path.exists(board_dir):
        return None
    
    # Buscar carpetas que empiecen con el thread_id
    for item in os.listdir(board_dir):
        if item.startswith(f"{thread_id}_") and os.path.isdir(os.path.join(board_dir, item)):
            thread_folder = os.path.join(board_dir, item)
            html_files = [f for f in os.listdir(thread_folder) if f.endswith('.html')]
            if html_files:
                return {
                    "folder": thread_folder,
                    "html_file": os.path.join(thread_folder, html_files[0]),
                    "media_folder": os.path.join(thread_folder, MEDIA_SUBFOLDER)
                }
    return None

def get_thread_stats(thread_folder):
    """Obtiene estadísticas del hilo existente."""
    html_file = None
    for f in os.listdir(thread_folder):
        if f.endswith('.html'):
            html_file = os.path.join(thread_folder, f)
            break
    
    if not html_file or not os.path.exists(html_file):
        return None
    
    # Extraer número de posts del HTML existente
    try:
        with open(html_file, 'r', encoding='utf-8') as f:
            content = f.read()
            posts_match = re.search(r'📝 (\d+)</span>', content)
            if posts_match:
                return {"posts": int(posts_match.group(1))}
    except:
        pass
    
    return {"posts": 0}

def update_thread(board_name, thread_id, theme='light', config=None):
    """Actualiza un hilo existente con nuevos posts."""
    if config is None:
        config = load_config()
    
    existing = find_existing_thread(board_name, thread_id)
    if not existing:
        Console.print_warning(f"Hilo {thread_id} no encontrado localmente. Use descarga normal.")
        return download_thread(board_name, thread_id, theme, False, 0, config)
    
    try:
        board = basc_py4chan.Board(board_name)
        thread = board.get_thread(thread_id)
        if not thread:
            Console.print_warning(f"El hilo {thread_id} en /{board_name}/ ya no existe en 4chan.")
            return None
        
        # Comparar número de posts
        old_stats = get_thread_stats(existing["folder"])
        new_posts_count = len(thread.all_posts)
        old_posts_count = old_stats["posts"] if old_stats else 0
        
        if new_posts_count <= old_posts_count:
            Console.print_info(f"    > Hilo {thread_id} ya está actualizado ({old_posts_count} posts)")
            return {"id": thread_id, "subject": "Actualizado", "new_posts": 0}
        
        new_posts = new_posts_count - old_posts_count
        Console.print_info(f"    > Actualizando hilo {thread_id}: {new_posts} nuevos posts")
        
        # Descargar nuevos medios
        command = [
            'gallery-dl',
            '--directory', existing["media_folder"],
            '-o', 'path=.',
            '-o', 'filename={filename}.{extension}',
            '--write-info-json'
        ]
        ...
        try:
            ...
            Console.print_warning(f"    > Timeout en actualización del hilo {thread_id}")
            return None
        
        # Regenerar HTML completo
        thread_subject = thread.topic.subject if thread.topic.subject else "Sin_Asunto"
        downloaded_files = set(os.listdir(existing["media_folder"])) if os.path.exists(existing["media_folder"]) else set()
        html_content = create_thread_html(thread, board_name, thread_id, thread_subject, downloaded_files, theme)
        
        # Escribir HTML actualizado
        with open(existing["html_file"], "w", encoding="utf-8") as f:
            f.write(html_content)
        
        Console.print_success(f"    > Hilo {thread_id} actualizado con {new_posts} nuevos posts")
        return {"id": thread_id, "subject": thread_subject, "new_posts": new_posts}
        
    except Exception as e:
        Console.print_error(f"Error actualizando hilo {thread_id}: {str(e)}")
        return None

def update_board_threads(board_name, theme='light', max_updates=None):
    """Actualiza todos los hilos existentes de un board."""
    config = load_config()
    board_dir = os.path.join(BASE_DOWNLOAD_DIR, board_name)
    
    if not os.path.exists(board_dir):
        Console.print_warning(f"No se encontraron hilos de /{board_name}/ para actualizar")
        return
    
    # Encontrar todos los hilos existentes
    existing_threads = []
    for item in os.listdir(board_dir):
        if os.path.isdir(os.path.join(board_dir, item)) and '_' in item:
            try:
                thread_id = int(item.split('_')[0])
                existing_threads.append(thread_id)
            except ValueError:
                continue
    
    if not existing_threads:
        Console.print_warning(f"No se encontraron hilos válidos de /{board_name}/ para actualizar")
        return
    
    # Limitar número de actualizaciones si se especifica
    if max_updates and len(existing_threads) > max_updates:
        existing_threads = existing_threads[:max_updates]
        Console.print_info(f"Limitando a {max_updates} actualizaciones")
    
    Console.print_header(f"🔄 Actualizando {len(existing_threads)} hilos de /{board_name}/")
    
    start_time = time.time()
    results = []
    updated_count = 0
    
    for thread_id in tqdm(existing_threads, desc="Actualizando hilos"):
        result = update_thread(board_name, thread_id, theme, config)
        results.append(result)
        if result and result.get("new_posts", 0) > 0:
            updated_count += 1
    ...
    Console.print_success(f"✅ Hilos verificados: {len(existing_threads)}")
    Console.print_success(f"🔄 Hilos actualizados: {updated_count}")
    Console.print_success(f"📝 Nuevos posts: {total_new_posts}")
    Console.print_info(f"⏱️  Tiempo total: {elapsed:.1f} segundos")
...
Ejemplos de uso:
  ...
  %(prog)s "https://boards.4chan.org/g/" --update-all
    ...
    if args.update_all:
        ...
        if not board_match:
            Console.print_error("URL debe ser de un board para --update-all")
            sys.exit(1)
        
        board_name = board_match.group(1)
        update_board_threads(board_name, args.tema, args.max_updates)
        return
    
    # Require URL for other operations
    if not args.url:
        parser.print_help()
        Console.print_error("Se requiere URL para descargas y actualizaciones")
        sys.exit(1)
    ...
    if thread_match:
        ...
        if args.update:
            result = update_thread(board_name, thread_id, args.tema)
            if result:
                new_posts = result.get("new_posts", 0)
                if new_posts > 0:
                    Console.print_success(f"✅ Hilo actualizado con {new_posts} nuevos posts")
                else:
                    Console.print_success("✅ Hilo ya estaba actualizado")
        else:
            result = download_thread(board_name, thread_id, args.tema, args.overwrite, args.sleep)
            if result:
